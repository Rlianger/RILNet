...
from data.masks_transforms import masks_preprocess_all, AddBackgroundMask, ResizeMasks, PermuteMasksDim, \
    RemoveBackgroundMask
from data.data_augmentation import RandomOcclusion
...
def build_transforms(
    ...
    **kwargs
    ...
    if 'color_jitter' in transforms or 'cj' in transforms:
        print('+ color jitter')
        transform_tr += [
            ColorJitter(brightness=config.cj.brightness,
                        contrast=config.cj.contrast,
                        saturation=config.cj.saturation,
                        hue=config.cj.hue,
                        always_apply=config.cj.always_apply,
                        p=config.cj.p,
                        )
        ]
    ...
    if remove_background_mask:  # ISP masks
        ...
        if masks_preprocess != 'none':
            ...
            transform_tr += [masks_preprocess_transform()]  # CombinePifPafIntoEightBodyMasks
